...
90.2,14.1,9.5,2024-01-01,athlete_3"""
    ...
    def test_load_data_success(self, mock_read_csv):
        ...
        mock_df = pd.DataFrame(
            {"pitch_speed_mph": [85.0, 87.5, 90.2], "peak_velocity_x": [12.5, 13.2, 14.1]}
        )
...
class TestDataPreprocessing(unittest.TestCase):
    ...
    def test_filter_columns_custom(self):
        ...
        result = filter_columns(
            self.sample_data, include_patterns=["velocity"], additional_columns=["athlete_uid"]
        )
    ...
    def test_split_and_scale_data(self):
        ...
        # Check shapes
        total_samples = len(X)
        self.assertEqual(len(train_features_scaled) + len(test_features_scaled), total_samples)
        self.assertEqual(len(train_targets) + len(test_targets), total_samples)

        # Check that test size is approximately 20%
        self.assertAlmostEqual(len(test_features_scaled) / total_samples, 0.2, places=1)
...
class TestModelTraining(unittest.TestCase):
    ...
    def test_train_random_forest(self):
        ...
        # 'number_of_trees'
        model = train_random_forest(self.X_train, self.y_train, number_of_trees=10)
...
class TestVisualization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = pd.DataFrame({"pitch_speed_mph": np.random.normal(90, 5, 100)})
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_single_row_dataframe(self):
        ...
        single_row_df = pd.DataFrame({"pitch_speed_mph": [90.0], "peak_velocity_x": [13.0]})
    ...
    def test_no_numeric_columns(self):
        ...
        text_only_df = pd.DataFrame(
            {"name": ["Alice", "Bob", "Charlie"], "category": ["A", "B", "C"]}
        )
