...
def filter_columns(data: pd.DataFrame,
    ...
    for pattern in include_patterns:
        pattern_matched_cols.extend(
            [col for col in data.columns if pattern.lower() in col.lower()])

    # Combine with additional columns, only keeping existing ones
    all_desired_cols = []
    all_desired_cols.extend(pattern_matched_cols)
    all_desired_cols.extend(
        [col for col in additional_cols if col in data.columns])
...
def remove_outliers(data: pd.DataFrame,
    ...
    print(
        f"Removed {removed_count} outliers from '{column}' using {method} method")
...
def prepare_features_target(data: pd.DataFrame,
                            target_column: str = 'pitch_speed_mph') -> Tuple[pd.DataFrame,
                                                                             pd.Series]:
    """
...
def plot_actual_vs_predicted(y_test: pd.Series,
    ...
    plt.plot([y_test.min(), y_test.max()], [
             y_test.min(), y_test.max()], 'r--', label='1:1 line')
    plt.legend()
