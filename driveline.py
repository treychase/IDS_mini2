...
def filter_columns(
    dataset: pd.DataFrame, include_patterns: List[str] = None, additional_columns: List[str] = None
) -> pd.DataFrame:
    """
    ...
    all_desired_columns.extend([col for col in additional_columns if col in dataset.columns])
...
def remove_outliers(
    dataset: pd.DataFrame, target_column: str, method: str = "iqr", threshold_factor: float = 1.5
) -> pd.DataFrame:
    """
    ...
    print(f"Removed {outliers_removed_count} outliers from '{target_column}' using {method} method")

    return cleaned_dataset


def handle_missing_values(dataset: pd.DataFrame, imputation_strategy: str = "mean") -> pd.DataFrame:
    """
...
def prepare_features_target(
    dataset: pd.DataFrame, target_column_name: str = "pitch_speed_mph"
...
def train_linear_regression(
    train_features: np.ndarray, train_targets: pd.Series
) -> LinearRegression:
    """
...
def train_bayesian_ridge(train_features: np.ndarray, train_targets: pd.Series) -> BayesianRidge:
    """
...
def train_random_forest(
    ...
    """
    forest_model = RandomForestRegressor(n_estimators=number_of_trees, random_state=random_seed)
...
def evaluate_model(
    trained_model, test_features: np.ndarray, test_targets: pd.Series, model_display_name: str
) -> Dict[str, float]:
    """
...
def plot_actual_vs_predicted(
    ...
    plt.plot(
        [actual_targets.min(), actual_targets.max()],
        [actual_targets.min(), actual_targets.max()],
        "r--",
        label="1:1 line",
    )
...
def plot_model_comparison(
    ...
    if marker_mapping is None:
        marker_mapping = {"Linear Regression": "o", "Bayesian Ridge": "^", "Random Forest": "s"}
...
def train_all_models(train_features_scaled: np.ndarray, train_targets: pd.Series) -> Dict[str, any]:
    """
...
def evaluate_all_models(
    trained_models_dict: Dict[str, any], test_features_scaled: np.ndarray, test_targets: pd.Series
) -> List[Dict]:
    """
    ...
    bayesian_ridge_results = evaluate_model(
        trained_models_dict["bayesian_ridge"], test_features_scaled, test_targets, "Bayesian Ridge"
    )

    random_forest_results = evaluate_model(
        trained_models_dict["random_forest"], test_features_scaled, test_targets, "Random Forest"
    )
...
def create_model_visualizations(
    test_targets: pd.Series, evaluation_results_list: List[Dict]
) -> None:
    """
    ...
    plot_actual_vs_predicted(
        test_targets, evaluation_results_list[0]["predictions"], "Linear Regression", "blue"
    )

    plot_actual_vs_predicted(
        test_targets, evaluation_results_list[1]["predictions"], "Bayesian Ridge", "purple"
    )

    plot_actual_vs_predicted(
        test_targets, evaluation_results_list[2]["predictions"], "Random Forest", "green"
    )
...
def run_complete_pipeline(filepath: str = "hp_obp.csv") -> pd.DataFrame:
    ...
    train_features_scaled, test_features_scaled, train_targets, test_targets, feature_scaler = (
        split_and_scale_data(feature_matrix, target_vector)
    )
